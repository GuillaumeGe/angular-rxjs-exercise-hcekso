...
cars$
  .pipe(
    filter((car) => ['black', 'red'].includes(car.color)),
    map((car) => car.color),
    distinctUntilChange()
  )
  .subscribe((car) => {
...
/*
const trucksAndCarsCombined$ = combineLatest(
  cars$,
  trucks$
...
/*
const trucksAndCarsMerged$ = merge(
  cars$,
  trucks$
).subscribe(val => console.log(val));
*/
...
const trucksAndCarsMergedOrdered$ = merge(
  trucks$,
  cars$.pipe(delay(500))
).subscribe((val) => console.log(val));
...
// cars$
//   .pipe(
//     filter((c) => c.make === 'Ford' || c.make === 'Volvo'),
//     map((c) => c.make),
//     distinct(),
//     mergeMap((brand) => trucks$.pipe(filter((truck) => brand === truck.make)))
//   )
//   .subscribe((val) => console.log(val));

// #9 ---
// step 1: only emit the results of whoever emits first, if cars$ emits first then the trucks$ should be ignored completely
// const test2$ = race(trucks$, cars$).subscribe((val) =>
//   console.log(val)
// );
...
//const t$ = merge(trucks$.pipe(), cars$.pipe(delay(50))).subscribe((val) => console.log(val));
//const t2$ = cars$.pipe().subscribe((val) => console.log(val));

// #11 ---
// step 1: log something every 2000ms (the value can be a static value)
// interval(2000).pipe(map(() => "test")).subscribe((val) =>
//   console.log(val)
// );
